...
void CapabilityMap::loadFromMatlabBinFile(const string map_file)
{
  ...
  } else
  {
    ...
    this->log << "Loaded map_centre_left: " << this->map_centre_left[0] << ";"
        << this->map_centre_left[1] << ";" << this->map_centre_left[2] << '\n';
    inputFile.read((char *) this->map_centre_right.data(), sizeof(this->map_centre_right));
    this->log << "Loaded map_centre_right: " << this->map_centre_right[0] << ";"
        << this->map_centre_right[1] << ";" << this->map_centre_right[2] << '\n';
    ...
    this->log << "Loaded endeffector_link right: " << this->endeffector_link["right"].c_str()
        << endl;

    inputFile.read((char *) this->endeffector_axis.data(), sizeof(this->endeffector_axis));
    this->log << "Loaded endeffector_axis: " << this->endeffector_axis[0] << ";"
        << this->endeffector_axis[1] << ";" << this->endeffector_axis[2] << '\n';
    ...
    inputFile.read((char *) this->nominal_configuration.data(),
        this->n_joints * sizeof(VectorXd::Scalar));
    ...
    if (contains_map)
    {
      ...
      inputFile.read((char *) this->reachability_index.data(),
          this->n_voxels * sizeof(VectorXd::Scalar));
      ...
      this->log << "Loaded map_lower_bound: " << this->map_lower_bound[0] << ";"
          << this->map_lower_bound[1] << ";" << this->map_lower_bound[2] << '\n';

      inputFile.read((char *) this->map_upper_bound.data(), sizeof(this->map_upper_bound));
      this->log << "Loaded map_upper_bound: " << this->map_upper_bound[0] << ";"
          << this->map_upper_bound[1] << ";" << this->map_upper_bound[2] << '\n';
    } else
    {
    ...
    if (contains_occupancy_map)
    {
      ...
      inputFile.read((char *) this->occupancy_map_dimensions.data(),
          sizeof(this->occupancy_map_dimensions));
      this->log << "Loaded occupancy_map_dimensions: " << this->occupancy_map_dimensions[0] << ";"
          << this->occupancy_map_dimensions[1] << ";" << this->occupancy_map_dimensions[2] << '\n';

      inputFile.read((char *) this->occupancy_map_lower_bound.data(),
          sizeof(this->occupancy_map_lower_bound));
      this->log << "Loaded occupancy_map_lower_bound: " << this->occupancy_map_lower_bound[0] << ";"
          << this->occupancy_map_lower_bound[1] << ";" << this->occupancy_map_lower_bound[2]
          << '\n';

      inputFile.read((char *) this->occupancy_map_upper_bound.data(),
          sizeof(this->occupancy_map_upper_bound));
      this->log << "Loaded occupancy_map_upper_bound: " << this->occupancy_map_upper_bound[0] << ";"
          << this->occupancy_map_upper_bound[1] << ";" << this->occupancy_map_upper_bound[2]
          << '\n';
      ...
      inputFile.read((char *) this->occupancy_map_orient_steps["roll"].data(),
          n_roll_steps * sizeof(double));
      this->log << "Loaded occupancy_map_orient_steps.roll ("
          << this->occupancy_map_orient_steps["roll"].rows() << ")\n";
      ...
      inputFile.read((char *) this->occupancy_map_orient_steps["pitch"].data(),
          n_pitch_steps * sizeof(double));
      this->log << "Loaded occupancy_map_orient_steps.pitch ("
          << this->occupancy_map_orient_steps["pitch"].rows() << ")\n";
      ...
      inputFile.read((char *) this->occupancy_map_orient_steps["yaw"].data(),
          n_yaw_steps * sizeof(double));
      this->log << "Loaded occupancy_map_orient_steps.yaw ("
          << this->occupancy_map_orient_steps["yaw"].rows() << ")\n";
    } else
    {
    ...
    if (contains_map)
    {
      ...
      this->computeVoxelCentres(this->voxel_centres, this->map_lower_bound, this->map_upper_bound,
          this->voxel_edge);
    }
    if (contains_occupancy_map)
    {
      ...
      this->computeVoxelCentres(this->occupancy_voxel_centres, this->occupancy_map_lower_bound,
          this->occupancy_map_upper_bound, this->occupancy_map_resolution);
    }
...
void CapabilityMap::resetActiveVoxels(const bool include_zero_reachability)
{
  ...
  if (!include_zero_reachability)
  {
    ...
    idx.clear();
    boost::push_back(idx,
        boost::irange(0, (int) this->n_voxels) | boost::adaptors::filtered([this](int vox)
        { return this->reachability_index[vox] > 1e-6;}));
  }
...
{
  this->active_orientations = Matrix<bool, Dynamic, Dynamic>::Ones(this->n_voxels,
      this->n_occupancy_orient);
  for (int vox = 0; vox < this->n_voxels; vox++)
  {
    if (!this->active_voxels[vox])
    {
...
void CapabilityMap::reduceActiveSet(const bool reset_active, const vector<Vector3d> point_cloud,
    FPPOutput &output, const Vector2d sagittal_range, const Vector2d transverse_range,
    const Vector2d height_range, const double direction_threshold)
{
  ...
  Vector3d direction = quat2rotmat(this->endeffector_pose.block<4, 1>(3, 0))
      * this->endeffector_axis;
...
void CapabilityMap::deactivateVoxelsOutsideAngleRanges(Eigen::Vector2d sagittal_range,
    Eigen::Vector2d transverse_range, const bool reset_active)
{
  if (reset_active)
  {
    this->resetActiveVoxels();
  ...
  for (int vox = 0; vox < this->n_voxels; vox++)
  {
    if (this->active_voxels[vox])
    {
      ...
      if (sagittal_angle > sagittal_range(0) && sagittal_angle < sagittal_range(1)
          || transverse_angle > transverse_range(0) && transverse_angle < transverse_range(1))
      {
...
void CapabilityMap::deactivateVoxelsOutsideBaseHeightRange(const Eigen::Vector2d range,
    const bool reset_active)
{
  Vector3d position;
  ...
  for (int vox = 0; vox < this->n_voxels; vox++)
  {
    if (this->active_voxels[vox])
    {
      for (int orient = 0; orient < this->n_occupancy_orient; orient++)
      {
        ...
        {
          position = rpy2rotmat(this->occupancy_map_orientations[orient])
              * (this->voxel_centres[vox] + this->endeffector_pose.block<3, 1>(0, 0)
                  - this->map_centre);
...
void CapabilityMap::deactivateVoxelsByDirection(const Vector3d direction,
    const double direction_threshold, const bool reset_active)
{
  if (reset_active)
  {
    this->resetActiveVoxels();
  ...
  {
    for (const int &idx : this->findVoxelsFromDirection(rpy2rotmat(orient).transpose() * direction,
        direction_threshold))
    {
...
void CapabilityMap::deactivateCollidingVoxels(const vector<Vector3d> point_cloud,
    const bool reset_active)
{
  if (reset_active)
  {
  ...
  for (int point = 0; point < point_cloud.size(); point++)
  {
        ...
        && ((Array3d) point_cloud[point] < (Array3d) upper_bound).all())
    {
      ...
      transform(sub_d.data(), sub_d.data() + 3, sub_d.data(), ptr_fun((double (*)(double))floor));sub
      = sub_d.cast<int>();
      idx = sub(2) * this->occupancy_map_dimensions(1) * this->occupancy_map_dimensions(0)
          + sub(1) * this->occupancy_map_dimensions(0) + sub(0);
      if (find(om_occupied_voxels.begin(), om_occupied_voxels.end(), idx)
          == om_occupied_voxels.end())
      {
    ...
    if (this->isActiveVoxel(vox))
    {
      for (int orient = 0; orient < this->n_occupancy_orient; orient++)
      {
        if (this->isActiveOrient(vox, orient))
        {
          ...
          {
            if (find(om_occupied_voxels.begin(), om_occupied_voxels.end(), om_vox)
                != om_occupied_voxels.end())
            {
              this->active_orientations(vox, orient) = false;
              break;
...
vector<unsigned int> CapabilityMap::findVoxelsFromDirection(const Vector3d direction,
    const double threshold, const bool active_set_only)
{
...
vector<unsigned int> CapabilityMap::findPointsFromDirection(Vector3d direction,
...
vector<Vector3d> CapabilityMap::distributePointsOnSphere()
{
  ...
  {
    phi.push_back(
        fmod(phi[i - 1] + 3.6 / (sqrt(this->n_directions_per_voxel) * sqrt(1 - pow(h[i], 2))),
            2 * M_PI));
  }
  phi.push_back(0);
  ...
  {
    points.push_back(
        Vector3d(cos(phi[i]) * sin(theta[i]), sin(phi[i]) * sin(theta[i]), cos(theta[i])));
  }
  return points;
}

void CapabilityMap::computeVoxelCentres(vector<Eigen::Vector3d> &centre_array,
    const Vector3d lower_bound, const Vector3d upper_bound, const double resolution)
{
  Vector3d dimensions_double = (upper_bound - lower_bound) / resolution;
  transform(dimensions_double.data(), dimensions_double.data() + 3, dimensions_double.data(),
      ptr_fun((double (*)(double))round));Vector3i
  dimensions = dimensions_double.cast<int>();
...
void CapabilityMap::computeProbabilityDistribution(const VectorXd mu, const VectorXd sigma)
{
  VectorXd sigma_inverse = ((ArrayXd) sigma).pow(-1);
  MatrixXd sigma_inverse_matrix = sigma_inverse.asDiagonal();
  int n_orient = this->getNActiveOrientations();
  this->voxel_probability.resize(n_orient);
  this->probability_orientations.resize(n_orient);
  this->probability_voxels.resize(n_orient);
  VectorXd values(6);
  int idx = 0;
  for (int vox = 0; vox < this->active_orientations.rows(); vox++)
  {
    for (int orient = 0; orient < this->active_orientations.cols(); orient++)
    {
      if (this->active_orientations(vox, orient))
      {
        values << this->voxel_centres[vox], this->occupancy_map_orientations[orient];
        this->probability_voxels[idx] = vox;
        this->probability_orientations[idx] = orient;
        this->voxel_probability(idx) = pow(pow(2 * M_PI, 6) * sigma.prod(), -.5)
            * (exp(-.5 * (values - mu).transpose() * sigma_inverse_matrix * (values - mu)));
        idx++;
      }
    }
  }
...
{
  if (this->voxel_probability.size() < 1)
  {
    return 13;
  }
  ArrayXd cumulative_probability(this->voxel_probability.size());
  partial_sum(this->voxel_probability.data(),
      this->voxel_probability.data() + this->voxel_probability.size(),
      cumulative_probability.data());
  ...
  {
    this->voxel_probability.block(idx, 0, this->voxel_probability.rows() - 1 - idx, 1) =
        this->voxel_probability.block(idx + 1, 0, this->voxel_probability.rows() - 1 - idx, 1);
  }
...
void CapabilityMap::setOccupancyMapOrientations()
{
  for (int yaw = 0; yaw < this->occupancy_map_orient_steps["yaw"].rows(); yaw++)
  {
    for (int pitch = 0; pitch < this->occupancy_map_orient_steps["pitch"].rows(); pitch++)
    {
      ...
      {
        this->occupancy_map_orientations.push_back(
            Vector3d(this->occupancy_map_orient_steps["roll"][roll],
...
void CapabilityMap::drawCapabilityMap(bot_lcmgl_t *lcmgl, const int orient, const Vector3d centre,
    const bool draw_cubes)
{
...
void CapabilityMap::drawActiveMap(bot_lcmgl_t *lcmgl, const int orient, const Vector3d centre,
    const bool draw_cubes)
{
  std::vector<unsigned int> idx;
  for (int vox = 0; vox < this->n_voxels; vox++)
...
void CapabilityMap::drawOccupancyMap(bot_lcmgl_t *lcmgl, const unsigned int capability_map_voxel,
    const unsigned int orient, const Eigen::Vector3d centre, const bool draw_cubes)
{
  if (draw_cubes)
  {
    this->drawMapCubes(lcmgl, this->occupancy_map_lower_bound, this->occupancy_map_upper_bound,
        this->occupancy_map_resolution, this->occupancy_map_orientations[orient], centre);
  }
  ...
  {
    cout << vox << endl;
    Vector3d point = rpy2rotmat(this->occupancy_map_orientations[orient])
        * this->occupancy_voxel_centres[vox] + centre;
...
void CapabilityMap::drawMap(bot_lcmgl_t *lcmgl, const vector<unsigned int> &voxels,
    const Vector3d orient, const Vector3d centre, const bool draw_cubes)
{
  int start_idx;
  if (draw_cubes)
  {
    this->drawMapCubes(lcmgl, this->map_lower_bound, this->map_upper_bound, this->voxel_edge,
        orient, centre);
    start_idx = 0;
  } else
  {
  ...
  for (int i = start_idx; i < this->n_directions_per_voxel; i++)
  {
    ...
    {
      if (abs((float) this->reachability_index(vox) - (float) i / this->n_directions_per_voxel)
          < 1e-6)
      {
...
void CapabilityMap::drawMapCubes(bot_lcmgl_t *lcmgl, const Vector3d lb, const Vector3d ub,
    const double resolution, const Vector3d orient, const Vector3d centre)
{
